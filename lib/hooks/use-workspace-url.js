...
export const DEFAULT_TAB = "Overview (P0)";
...
export function useWorkspaceUrl() {
  ...
  // Generic open-entity selection for the reusable Entity engine (Properties,
  // Tenants, Maintenance, Leasing…). Mirrors `event`, but area-agnostic — only
  // one editor is open per tab, and the tab already scopes the path.
  ...
  const buildUrl = useCallback(
    (next) => {
      ...
      const it = next.item !== undefined ? next.item : itemId;
    ...
    [projectId, tab, eventId, itemId, section, workflowId, venueId, pathname],
  );
  ...
  // Switching workspace tabs exits any open event/item/section/workflow/venue.
  ...
  // Generic open/close for the Entity engine (mirrors openEvent/closeEvent).
  // Opening resets to the item's default section.
  const openItem = useCallback(
    (id) => apply({ item: id, section: null }),
    [apply],
  );
  const closeItem = useCallback(
    () => apply({ item: null, section: null }),
    [apply],
  );
  // Switch to another tab and open an item there in one navigation — used when a
  // section (e.g. a property's Units list) hands off to another area's editor.
  const openItemInTab = useCallback(
    (id, nextTab) => apply({ tab: nextTab, item: id, section: null }),
    [apply],
  );
